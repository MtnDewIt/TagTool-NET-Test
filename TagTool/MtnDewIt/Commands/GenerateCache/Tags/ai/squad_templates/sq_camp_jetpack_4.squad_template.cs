...
{
    public class ai_squad_templates_sq_camp_jetpack_4_squad_template : TagFile
    {
        public ai_squad_templates_sq_camp_jetpack_4_squad_template(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = GetCachedTag<SquadTemplate>($@"ai\squad_templates\sq_camp_jetpack_4");
            var sqtm = CacheContext.Deserialize<SquadTemplate>(Stream, tag);
            sqtm.Name = CacheContext.StringTable.GetOrAddString($@"sq_camp_brute_jetpack_4");
            sqtm.CellTemplates = new List<SquadTemplate.CellTemplate>
            {
                new SquadTemplate.CellTemplate
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"4_jetpack"),
                    NormalDiffCount = 4,
