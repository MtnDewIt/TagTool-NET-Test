...
{
    public class ai_squad_templates_sq_camp_captain_jackal_2_squad_template : TagFile
    {
        public ai_squad_templates_sq_camp_captain_jackal_2_squad_template(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = GetCachedTag<SquadTemplate>($@"ai\squad_templates\sq_camp_captain_jackal_2");
            var sqtm = CacheContext.Deserialize<SquadTemplate>(Stream, tag);
            sqtm.Name = CacheContext.StringTable.GetOrAddString($@"sq_camp_cap_jackal_2");
            sqtm.CellTemplates = new List<SquadTemplate.CellTemplate>
            {
                ...
                new SquadTemplate.CellTemplate
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"2_jackal"),
                    NormalDiffCount = 2,
                    Characters = new List<SquadTemplate.CellTemplate.ObjectBlock>
                    {
                        new SquadTemplate.CellTemplate.ObjectBlock
