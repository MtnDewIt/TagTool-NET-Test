...
{
    public class ai_squad_templates_sq_camp_grunt_flak_3_squad_template : TagFile
    {
        public ai_squad_templates_sq_camp_grunt_flak_3_squad_template(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = GetCachedTag<SquadTemplate>($@"ai\squad_templates\sq_camp_grunt_flak_3");
            var sqtm = CacheContext.Deserialize<SquadTemplate>(Stream, tag);
            sqtm.Name = CacheContext.StringTable.GetOrAddString($@"sq_camp_grunt_flak_3");
