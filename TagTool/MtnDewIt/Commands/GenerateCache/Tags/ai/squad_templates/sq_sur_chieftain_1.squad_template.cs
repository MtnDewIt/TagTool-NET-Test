...
{
    public class ai_squad_templates_sq_sur_chieftain_1_squad_template : TagFile
    {
        public ai_squad_templates_sq_sur_chieftain_1_squad_template(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = GetCachedTag<SquadTemplate>($@"ai\squad_templates\sq_sur_chieftain_1");
            var sqtm = CacheContext.Deserialize<SquadTemplate>(Stream, tag);
            sqtm.Name = CacheContext.StringTable.GetOrAddString($@"sq_sur_chieftain_1");
