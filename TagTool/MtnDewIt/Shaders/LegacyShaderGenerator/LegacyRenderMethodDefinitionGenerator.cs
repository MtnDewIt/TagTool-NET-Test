    ...
    public class LegacyRenderMethodDefinitionGenerator 
    {
                        ...
                        for (int j = 0; j < pass.CategoryDependencies.Count; j++)
                        {
                            ...
                            foreach (var dep in glps.EntryPoints[(int)rmdf.EntryPoints[i].EntryPoint].CategoryDependency)
                            {
                                if (dep.DefinitionCategoryIndex == pass.CategoryDependencies[j].Category)
                                {
                                    ...
                                    {
                                        glps = LegacyShaderGenerator.GenerateSharedPixelShader(cache, generator);
        ...
        private static RenderMethodOption BuildRenderMethodOptionFromParameters(GameCache cache, ShaderParameters parameters)
        {
            ...
            foreach (var parameter in parameters.Parameters)
            {
                ...
                parameterBlock.RenderMethodExtern = (TagTool.Shaders.RenderMethodExtern)parameter.RenderMethodExtern;
        ...
        private static void GenerateSharedShaders(GameCache cache, Stream cacheStream, IShaderGenerator generator, string shaderType, out CachedTag glpsTag, out CachedTag glvsTag, out GlobalPixelShader glps, out GlobalVertexShader glvs)
        {
            glps = LegacyShaderGenerator.GenerateSharedPixelShader(cache, generator);

            if (shaderType != "beam" && shaderType != "contrail" && shaderType != "light_volume" && shaderType != "particle" && shaderType != "screen")
            {
                glvs = LegacyShaderGenerator.GenerateSharedVertexShader(cache, generator);
            }
            else
            {
