...
namespace TagTool.MtnDewIt.Shaders.LegacyShaderMatching
{
    public class LegacyShaderMatcherNew 
    {
        ...
        public CachedTag FindClosestTemplate(CachedTag sourceRmt2Tag, RenderMethodTemplate sourceRmt2, bool canGenerate)
        {
            ...
            {
                if (!LegacyRenderMethodDefinitionGenerator.UpdateRenderMethodDefinition(BaseCache, BaseCacheStream, sourceRmt2Desc.Type))
        ...
        private bool TryGenerateTemplate(string tagName, Rmt2Descriptor rmt2Desc, out CachedTag generatedRmt2)
        {
            ...
            var generator = rmt2Desc.GetLegacyGenerator(true);
            if (generator == null)
                return false;
            ...
            if (!BaseCache.TagCache.TryGetTag($"shaders\\{rmt2Desc.Type}.rmdf", out rmdfTag))
            {
                ...
                rmdf = LegacyRenderMethodDefinitionGenerator.GenerateRenderMethodDefinition(BaseCache, BaseCacheStream, generator, rmt2Desc.Type, out glps, out glvs);
        ...
        public struct Rmt2Descriptor
        {
            ...
            public IShaderGenerator GetLegacyGenerator(bool applyFixes = false)
            {
                if (HasParsed && !IsMs30)
                {
                    switch (Type)
                    {
                        ...
                        //case "glass": return new HaloShaderGenerator.LegacyGenerator.Glass.LegacyGlassGenerator(Options, applyFixes);
