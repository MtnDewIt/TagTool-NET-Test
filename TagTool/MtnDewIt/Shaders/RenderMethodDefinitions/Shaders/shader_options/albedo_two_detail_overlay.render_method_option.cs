...
{
    public class shaders_shader_options_albedo_two_detail_overlay_render_method_option : RenderMethodData
    {
        public shaders_shader_options_albedo_two_detail_overlay_render_method_option(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = Cache.TagCache.GetTag<RenderMethodOption>($@"shaders\shader_options\albedo_two_detail_overlay");
            ...
            rmop.Parameters = new List<RenderMethodOption.ParameterBlock>
            {
                ...
                new RenderMethodOption.ParameterBlock
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"detail_map_overlay"),
                    Type = RenderMethodOption.ParameterBlock.OptionDataType.Bitmap,
                    RenderMethodExtern = RenderMethodExtern.none,
                    DefaultSamplerBitmap = Cache.TagCache.GetTag<Bitmap>($@"shaders\default_bitmaps\bitmaps\default_detail"),
                    DefaultFloatArgument = 0f,
