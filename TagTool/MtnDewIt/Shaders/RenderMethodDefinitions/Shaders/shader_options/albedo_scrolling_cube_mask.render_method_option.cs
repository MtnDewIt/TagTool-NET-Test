...
{
    public class shaders_shader_options_albedo_scrolling_cube_mask_render_method_option : RenderMethodData
    {
        public shaders_shader_options_albedo_scrolling_cube_mask_render_method_option(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = Cache.TagCache.GetTag<RenderMethodOption>($@"shaders\shader_options\albedo_scrolling_cube_mask");
            ...
            rmop.Parameters = new List<RenderMethodOption.ParameterBlock>
            {
                ...
                new RenderMethodOption.ParameterBlock
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"color_blend_mask_cubemap"),
                    Type = RenderMethodOption.ParameterBlock.OptionDataType.Bitmap,
                    RenderMethodExtern = RenderMethodExtern.none,
                    DefaultSamplerBitmap = Cache.TagCache.GetTag<Bitmap>($@"shaders\default_bitmaps\bitmaps\gray_50_percent"),
                    DefaultFloatArgument = 0f,
                ...
                new RenderMethodOption.ParameterBlock
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"albedo_second_color"),
                    Type = RenderMethodOption.ParameterBlock.OptionDataType.ArgbColor,
                    RenderMethodExtern = RenderMethodExtern.none,
                    DefaultSamplerBitmap = null,
                    DefaultFloatArgument = 1f,
