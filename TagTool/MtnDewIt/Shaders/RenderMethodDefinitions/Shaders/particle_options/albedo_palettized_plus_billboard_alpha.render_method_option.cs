...
{
    public class shaders_particle_options_albedo_palettized_plus_billboard_alpha_render_method_option : RenderMethodData
    {
        public shaders_particle_options_albedo_palettized_plus_billboard_alpha_render_method_option(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = Cache.TagCache.GetTag<RenderMethodOption>($@"shaders\particle_options\albedo_palettized_plus_billboard_alpha");
            ...
            rmop.Parameters = new List<RenderMethodOption.ParameterBlock>
            {
                ...
                new RenderMethodOption.ParameterBlock
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"alpha_map"),
                    Type = RenderMethodOption.ParameterBlock.OptionDataType.Bitmap,
                    RenderMethodExtern = RenderMethodExtern.none,
                    DefaultSamplerBitmap = Cache.TagCache.GetTag<Bitmap>($@"shaders\default_bitmaps\bitmaps\alpha_grey50"),
                    DefaultFloatArgument = 0f,
                    DefaultIntBoolArgument = 0,
                    Flags = 0,
                    DefaultFilterMode = RenderMethodOption.ParameterBlock.DefaultFilterModeValue.Trilinear,
