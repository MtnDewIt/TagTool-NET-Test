...
{
    public class shaders_particle_options_frame_blend_on_render_method_option : RenderMethodData
    {
        public shaders_particle_options_frame_blend_on_render_method_option(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = Cache.TagCache.GetTag<RenderMethodOption>($@"shaders\particle_options\frame_blend_on");
            ...
            rmop.Parameters = new List<RenderMethodOption.ParameterBlock>
            {
                ...
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"starting_uv_scale"),
                ...
                new RenderMethodOption.ParameterBlock
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"ending_uv_scale"),
                    Type = RenderMethodOption.ParameterBlock.OptionDataType.Real,
                    RenderMethodExtern = RenderMethodExtern.none,
                    DefaultSamplerBitmap = null,
                    DefaultFloatArgument = 1f,
