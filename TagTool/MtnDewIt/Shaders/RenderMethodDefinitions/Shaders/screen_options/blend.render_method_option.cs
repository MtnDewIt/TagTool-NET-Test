...
{
    public class shaders_screen_options_blend_render_method_option : RenderMethodData
    {
        public shaders_screen_options_blend_render_method_option(GameCache cache, GameCacheHaloOnline cacheContext, Stream stream) : base
        (
            cache,
            cacheContext,
            stream
        ...
        {
            var tag = Cache.TagCache.GetTag<RenderMethodOption>($@"shaders\screen_options\blend");
            ...
            rmop.Parameters = new List<RenderMethodOption.ParameterBlock>
            {
                ...
                {
                    Name = CacheContext.StringTable.GetOrAddString($@"fade"),
